...
V2_STABLE_DIFFUSION_PATH = "stabilityai/stable-diffusion-2"     # use only the tokenizer from here
...
def interrogate(args):
  ...
  # check if LoRA has weights for the text encoder (ideally should be validated on the LoRA side)
  has_te_weight = False
  ...
  logger.error("This LoRA does not have modules for the Text Encoder; cannot interrogate.")
  return
  del vae
  ...
  unet.eval()               # we won't call U-Net here, but keep eval for consistency

  # iterate over tokens one by one
  token_id_start = 0
    ...
    with torch.no_grad():
      for token_id in tqdm(range(token_id_start, token_id_end + 1, args.batch_size)):
        ...
        for tid in range(token_id, min(token_id_end + 1, token_id + args.batch_size)):
          ...
          # tokens = [tid]                                                    # this variant gave poorer results
          batch.append(tokens)

  # batch_embs = text_encoder(torch.tensor(batch).to(DEVICE))[0].to("cpu")  # including bos/eos seems to produce more contrast [:, 1]
  # handle clip skip
  ...
  logger.info("You can ignore warnings starting with '_IncompatibleKeys' (older LoRA models may not define 'alpha').")
  logger.info("get text encoder embeddings with lora.")
  lora_embs = get_all_embeddings(text_encoder)

  # compare: use mean absolute difference for now
  logger.info("comparing...")
  diffs = {}
  for i, (orig_emb, lora_emb) in enumerate(zip(orig_embs, tqdm(lora_embs))):
    diff = torch.mean(torch.abs(orig_emb - lora_emb))
  # diff = torch.mean(torch.cosine_similarity(orig_emb, lora_emb, dim=1))       # didn't work well for detection
